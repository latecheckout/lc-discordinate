...
CREATE OR REPLACE FUNCTION auth.is_session_button_pressing_active(_session_id uuid, _time timestamp with time zone)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN EXISTS (
        ...
        AND _time >= s.scheduled_at + (sc.countdown_seconds || ' seconds')::interval
        AND _time < s.scheduled_at + (sc.countdown_seconds + sc.button_press_seconds || ' seconds')::interval
    );
END;
$function$
;
...
BEGIN 
    ...
    RETURN EXISTS ( 
        ...
        AND created_at >= NOW() - (_button_press_timeout_seconds || ' seconds')::interval
    ); 
END; 
$function$
;
